...
var dom = {
  ...
  hasClass: function hasClass(el, classNames) {
    ...
    (0, _traversal.foreach)(classNames.split(' ').map(function (c) {
      ...
      if ((0, _array.inArray)(c, Array.from(el.classList))) return;
      foundClasses = false;
      return false;
    });
    return foundClasses;
  ...
  hide: function hide(el) {
    if (undefined === this.data(el, '__display__')) {
      var display = '';
      if ((0, _is.isFunction)(window.getComputedStyle)) {
   ...
   * @returns {Element} The element
   */
  toggle: function toggle(el, state) {
    if (undefined !== state) {
    ...
    if ((0, _is.isString)(style)) {
      ...
      if (undefined === value) {
        if (window.getComputedStyle) {
          ...
          return computedStyle.getPropertyValue(style) || computedStyle[(0, _string.camelCase)(style)] || '';
        }
        return el.style[(0, _string.camelCase)(style)] || '';
      }
      if (prop.startsWith('--')) {
  ...
  offset: function offset(el) {
    if ((0, _is.isWindow)(el)) {
      return {
        top: el.scrollY,
        left: el.scrollX
      };
    } else if ((0, _is.isDocument)(el)) {
      return {
